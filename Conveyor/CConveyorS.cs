...
namespace Conveyor
{
    internal class CConveyorS : CDevice
    {
        ...
        private SERVO_POS curPos;   // 현재위치를 가져오는 변수
        ...
        public override void Process()
        {
            if (blsAutoConv == true)
            {
                switch (stepConv)
                {
                    ...
                    case 600:
                        if(carrier.id != 0)     //id가 0이면 1번 컨베이어로 보냄
                        {
                            if(carrier.use == CCarrier.USE.USE_TAKEOUT)     //자제정보가 takeout이면 1번으로 반출
                            {
                                if (ConvPio1.LReq)
                                {
                                    stepConv = 700;
                            ...
                            else if (carrier.use == CCarrier.USE.USE_STACK) //자제정보가 stack이면 3번이나 4번으로 적재
                            {
                                if (ConvPio3.LReq)
                                {
                                    stepConv = 900;
                            ...
                            else if (carrier.use == CCarrier.USE.USE_NONE)  //정보가 none면 1번으로 반출
                            {
                                //error처리
                                if (ConvPio1.LReq)
                                {
            ...
            if (oldStepConv != stepConv)
                Console.WriteLine("Shettle Conveyor Step = {0}", stepConv);
